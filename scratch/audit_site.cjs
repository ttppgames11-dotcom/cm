...
htmlFiles.forEach(file => {
  ...
  while ((match = imgRegex.exec(content)) !== null) {
    ...
    if (!fs.existsSync(absPath)) {
      issues.push(`[${file}] Broken IMG src: "${match[1]}"`);
    }
  }
  ...
  while ((match = linkRegex.exec(content)) !== null) {
    ...
    const absPath = path.resolve(root, src);
    if (!fs.existsSync(absPath)) {
      issues.push(`[${file}] Broken CSS/JS: "${match[2]}"`);
    }
  }
  ...
  while ((match = aRegex.exec(content)) !== null) {
    let href = match[1];
    if (href.startsWith('#') || href.startsWith('mailto:') || href.startsWith('tel:') || href.startsWith('http') || href.startsWith('javascript:')) continue;
  ...
  // 4. Check CSS background-image url('...')
  ...
  while ((match = bgRegex.exec(content)) !== null) {
    ...
    const absPath = path.resolve(root, src);
    if (!fs.existsSync(absPath)) {
      issues.push(`[${file}] Broken inline background-image: "${match[1]}"`);
    }
  }
});
